...
/* With the local symbol, foo, we convert
   ...
   test %reg, foo@GOT[(%reg)]
   to
   test $foo, %reg
   and convert
   binop foo@GOT[(%reg)], %reg
   to
   binop $foo, %reg
...
convert_load:
	      if (opcode == 0x8b)
		{
		  /* Convert "mov foo@GOT(%reg), %reg" to
		     "lea foo@GOTOFF(%reg), %reg".  */
		  if (r_type == R_386_GOT32X
	      ...
	      else
		{
		  ...
		  if (opcode == 0x85)
		    {
		      /* Convert "test %reg, foo@GOT(%reg)" to
			 "test $foo, %reg".  */
		    ...
		    {
		      /* Convert "binop foo@GOT(%reg), %reg" to
			 "binop $foo, %reg".  */
		      modrm = (0xc0
			       | (modrm & 0x38) >> 3
			       | (opcode & 0x3c));
